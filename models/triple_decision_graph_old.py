"""
三支决策图模块 (Triple Decision Graph Module)
...
class TripleDecisionGraph(nn.Module):
    ...
    def __init__(self, n_concepts, embed_dim, n_layers=2, 
        ...
        Args:
            ...
            beta: 负域阈值（论文：0.3）
            max_k: 最大阶数（论文：2）
            distance_decay_lambda: 距离衰减系数 λ（论文：0.5）
            dropout: Dropout率
        """
        ...
        self.beta = beta
        self.max_k = max_k
        self.distance_decay_lambda = distance_decay_lambda
        
        # 概念嵌入层
        self.concept_embed = nn.Embedding(n_concepts, embed_dim)
        
        # 差异化消息传递MLP（论文公式0-13~0-16）
        # 为每个区域和每个阶数创建MLP
        ...
        # 区域间融合矩阵W_r（论文公式0-20）
        # 输入：[m_pos || m_bnd || m_neg] = [3*embed_dim]
        # 输出：[embed_dim]
        self.W_r = nn.ModuleDict()
        for k in range(1, max_k + 1):
            self.W_r[str(k)] = nn.Linear(3 * embed_dim, embed_dim, bias=False)
        
        # 跨阶融合矩阵W_h和偏置b_h（论文公式0-21）
        # 输入：[h_self || m_1hop || m_2hop] = [3*embed_dim]（假设max_k=2）
        # 输出：[embed_dim]
        self.W_h = nn.Linear((max_k + 1) * embed_dim, embed_dim, bias=True)
