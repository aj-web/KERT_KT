...
def main():
    ...
    for result in all_results:
        ...
        # 保存单个模型结果: {model_name}_{dataset}.json
        model_result_file = os.path.join(results_dir, f'{model_name}_{dataset_name}.json')
        model_data = {
            ...
            'test_metrics': {
                'auc': {
                    'mean': float(result['auc_mean']),
                    'std': float(result['auc_std']),
                    'all_runs': [float(x) for x in result['all_aucs']]
                },
                'acc': {
                    'mean': float(result['acc_mean']),
                    'std': float(result['acc_std']),
                    'all_runs': [float(x) for x in result['all_accs']]
                }
            }
        }
        with open(model_result_file, 'w', encoding='utf-8') as f:
            json.dump(model_data, f, indent=2, ensure_ascii=False)
        print(f"  [OK] Saved: {model_name}_{dataset_name}.json")
    
    # 2. 按数据集分组，保存对比汇总文件
    datasets_in_results = set(r['dataset'] for r in all_results)
    for dataset_name in datasets_in_results:
        dataset_results = [r for r in all_results if r['dataset'] == dataset_name]
        
        # 保存对比汇总: baseline_comparison_{dataset}.json
        comparison_file = os.path.join(results_dir, f'baseline_comparison_{dataset_name}.json')
        comparison_data = {
            'dataset': dataset_name,
            'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            'n_runs': args.n_runs,
        ...
        for result in dataset_results:
            ...
            comparison_data['models'][model_name] = {
                'auc_mean': float(result['auc_mean']),
                'auc_std': float(result['auc_std']),
                'acc_mean': float(result['acc_mean']),
                'acc_std': float(result['acc_std']),
                'all_aucs': [float(x) for x in result['all_aucs']],
                'all_accs': [float(x) for x in result['all_accs']]
            }
        
        with open(comparison_file, 'w', encoding='utf-8') as f:
            json.dump(comparison_data, f, indent=2, ensure_ascii=False)
        print(f"  [OK] Saved: baseline_comparison_{dataset_name}.json")
    
    # 3. 保存全局汇总（可选，方便查看所有结果）
    all_results_file = os.path.join(results_dir, 'all_baseline_results.json')
    all_results_data = {
        'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        'n_runs': args.n_runs,
        'n_epochs': args.n_epochs,
    ...
    print(f"  [OK] Saved: all_baseline_results.json")
